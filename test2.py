...
class MVPGeneticAlgorithm(GeneticAlgorithm):
    ...
    def init_population(self):
        ...
        for _ in range(self.pop_size):
            # Ensure both vehicle types are included
            vehicle_a = self.num_pts - 2  # ID for vehicle A
            vehicle_b = self.num_pts - 1  # ID for vehicle B
            
            # Create a permutation of customer IDs
            customer_permutation = np.random.permutation([i for i in range(1, self.num_pts-2)])
            
            # Insert vehicle IDs randomly
            insert_a = np.random.randint(0, len(customer_permutation))
            insert_b = np.random.randint(0, len(customer_permutation) + 1)
            
            # Ensure insert_b is different from insert_a
            while insert_b == insert_a:
                insert_b = np.random.randint(0, len(customer_permutation) + 1)
            
            # Always insert the larger index first to avoid shifting issues
            if insert_b > insert_a:
                individual = np.insert(customer_permutation, insert_b, vehicle_b)
                individual = np.insert(individual, insert_a, vehicle_a)
            else:
                individual = np.insert(customer_permutation, insert_a, vehicle_a)
                individual = np.insert(individual, insert_b, vehicle_b)
            
            # Add depot at start and end
            individual = np.hstack(([0], individual, [0]))
    ...
    def calc_fitness(self, individual, show_log=False):
        ...
        The fitness score is calculated by penalizing the total cost of travel and unmet demand.
        ...
        """
        invalid = False
        total_cost, total_distance, total_demand, unmet_demand = 0, 0, 0, 0
        assigned_cars = [-1 for _ in range(len(individual))]  # 1 index
        cur_car_id = -1
        individual_cars = [{"cost": 0, "distance": 0, "demand": 0} for _ in range(2)]  # Only two cars: A and B
        ...
        for i_r, j in enumerate(reversed(individual)):
            ...
            if j > len(self.custs):  # For car points
                cur_car_id = j
                car_type = self.cars[cur_car_id - 1 - len(self.custs)]['type']
                weight = self.car_types[car_type]['cost']
                distance = self.dist_matrix[individual[i-1] - 1, individual[i] - 1]
                assigned_cars[i] = cur_car_id
                total_cost += weight * distance
                total_distance += distance
                individual_cars[car_type]['distance'] += distance
                individual_cars[car_type]['cost'] += weight * distance
                ...
                individual_cars[car_type]['demand'] += self.custs[j - 1]['demand']
                individual_cars[car_type]['distance'] += distance
                individual_cars[car_type]['cost'] += weight * distance
                if individual_cars[car_type]['demand'] > self.car_types[car_type]['capacity']:
                    unmet_demand += individual_cars[car_type]['demand'] - self.car_types[car_type]['capacity']
        ...
        if total_cost == 0:
            ...
            fitness = 100 / (total_cost + unmet_demand * 10)  # Penalize unmet demand

        if show_log:
            if fitness > 0:
                print("Total distance = {0:.3f} km".format(total_distance))
                print("Total cost = RM {0:.2f}".format(total_cost))
                print("Total demand met = {0}".format(total_demand))
                print("Unmet demand = {0}".format(unmet_demand))
                print()
                car_cnt = 0
                for i, j in enumerate(self.cars):
                    ...
                    for ii, jj in enumerate(individual):
                        if assigned_cars[ii] != j['id']:
                            continue  # ignore those not belonging to me
...
cars = [
    {"id": len(custs) + 1, "type": 0},  # Vehicle A
    {"id": len(custs) + 2, "type": 1}   # Vehicle B
]
NUM_PTS = len(custs) + 3  # customers + depot + 2 vehicles
POP_SIZE = 300
SELECTION_SIZE = 4
ELITE_SIZE = 5
NUM_GENS = 20
